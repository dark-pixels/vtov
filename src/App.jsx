...
import {
    ShoppingBasket, Truck, Leaf, Users, Menu, X, Phone, Mail, MapPin, AlertCircle, ArrowRight, Sun, Droplets,
    Heart, Star, Sprout, Calendar, ChevronDown, CloudRain, Flower
} from 'lucide-react';
...
const FallingLeaves = ({ count = 15 }) => {
    return (
        <div className="fixed inset-0 pointer-events-none overflow-hidden z-0 h-full w-full">
            {[...Array(count)].map((_, i) => (
                <div key={i} className="absolute animate-falling-leaf opacity-40" style={{
...
const RevealOnScroll = ({ children, delay = 0, direction = 'up' }) => {
    ...
    return (
        <div ref={ref} style={{ transitionDelay: `${delay}ms` }} className={`transition-all duration-1000 ease-out transform ${isVisible ? 'opacity-100 translate-x-0 translate-y-0' : `opacity-0 ${getTransform()}`}`}>
            {children}
        </div>
    );
};
        ...
        <div
            ...
            <div
                ...
                <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
                    {[...Array(5)].map((_, i) => (
                        <div key={i} className="absolute animate-falling-leaf opacity-20" style={{
                            left: `${Math.random() * 100}%`,
